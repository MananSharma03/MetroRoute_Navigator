...
const stations = {
    ...
    "Yamuna_Bank~B": { "Botanical_Garden~B": 10, "Vaishali~B": 8, "Rajiv_Chowk~BY": 6 },
    "Rajiv_Chowk~BY": { "Yamuna_Bank~B": 6, "Moti_Nagar~B": 9, "AIIMS~Y": 7, "New_Delhi~YO": 1 },
    "Vaishali~B": { "Yamuna_Bank~B": 8 },
    "Moti_Nagar~B": { "Rajiv_Chowk~BY": 9, "Janak_Puri_West~BO": 7, "Rajouri_Garden~BP": 2 },
    ...
    "New_Delhi~YO": { "Rajiv_Chowk~BY": 1, "Chandni_Chowk~Y": 2, "Shivaji_Stadium~O": 2 }, 
    ...
    "Punjabi_Bagh_West~P": { "Rajouri_Garden~BP": 2, "Netaji_Subhash_Place~PR": 3 }
};
...
function findShortestDistance(source, destination) {
    ...
    return { distance: distances[destination], path: reconstructPath(previous, source, destination) };
}
...
function findShortestTime(source, destination) {
    ...
    document.getElementById('output').innerText = `Shortest time: ${result.distance} minutes\nPath: ${result.path.join(' -> ')}`;
}
...
function handleOption() {
    ...
    switch(option) {
        case 'shortestDistance':
            ...
            document.getElementById('output').innerText = `Shortest distance: ${shortestDistanceResult.distance} KM\n\nPath: ${shortestDistanceResult.path.join(' -> ')}`;
            break;
        case 'shortestTime':
            findShortestTime(source, destination);
            break;
        case 'shortestPathDistance':
            const shortestPathDistanceResult = findShortestDistance(source, destination);
            document.getElementById('output').innerText = `Shortest path (distance): ${shortestPathDistanceResult.path.join(' -> ')}\nDistance: ${shortestPathDistanceResult.distance} KM`;
            break;
        case 'shortestPathTime':
            const shortestPathTimeResult = findShortestDistance(source, destination);
            document.getElementById('output').innerText = `Shortest path (time): ${shortestPathTimeResult.path.join(' -> ')}\nTime: ${shortestPathTimeResult.distance} minutes`;
            break;
        case 'fareDistance':
            const fareDistanceResult = findShortestDistance(source, destination);
            document.getElementById('output').innerText = `Fare for shortest path (distance): ${getFare(fareDistanceResult.distance)} INR`;
            break;
        case 'fareTime':
            const fareTimeResult = findShortestDistance(source, destination);
            document.getElementById('output').innerText = `Fare for shortest path (time): ${getFare(fareTimeResult.distance)} INR`;
            break;
        default:
